...
load_data <- function() {
  ...
  ready_short_term_interest_rates <- process_data(filter(detailed_monetary_data, SUBJECT=='IR3TIB'), 397, 'Subject')
  ...
  ready_datasets <- list(
    ...
    ready_short_term_interest_rates
  )
  ...
  data = list()
  data[['monthly']] <- monthly_dataset; data[['quarterly']] <- quarterly_dataset

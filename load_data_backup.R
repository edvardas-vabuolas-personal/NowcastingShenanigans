...
uk_data1 <- read.csv(file = './oecd_data/economic_outlook1.csv', head = T)
uk_data2 <- read.csv(file = './oecd_data/economic_outlook2.csv', head = T)
...
monetary_data <- read.csv('./oecd_data/monetary_financial_stats.csv', head=TRUE)

vars_vals <- list()

for (i in 1:nrow(monetary_data)) {
  # Pre-define variable name for optimization
  var_name <- monetary_data[i, "Subject"]
  
  # If variable is not in the list - create a new dict key with initial value
  if (length(vars_vals[[var_name]]) == 0) {
    vars_vals[[var_name]] <- list(monetary_data[i, "Value"])
  } else {
    # If variable is in the list - append dict value with new value
    vars_vals[[var_name]] <-
      append(vars_vals[[var_name]], monetary_data[i, "Value"])
  }
}

# Initiate data frame with 166 rows (1979Q1-2020Q1 has 504 months)
df <- as.data.frame(matrix(0, nrow = 504, ncol = 0))
# Iterate over dictionary keys
for (i in ls(vars_vals)) {
  # Ignore variable if there are missing values
  if (length(vars_vals[[i]]) == 504) {
    # Append data frame with a new column "i" with it's associated values as rows
    df[[i]] <- vars_vals[[i]]
  } else {
    print(paste(
